<?php
    // Database Installation Script for Cure Booking System
    // Author: Generated for Cure Booking System
...
<head>
    ...
    <title>Cure Booking Database Installation</title>
    <style>
        body {
...
<body>
    <div class="container">
        <h1>🏥 Cure Booking Database Installation</h1>
        
        <div class="config-section">
            <h3>📋 Database Configuration</h3>
            <p><strong>Host:</strong> <code><?php echo $host; ?></code></p>
            <p><strong>Username:</strong> <code><?php echo $username; ?></code></p>
            <p><strong>Password:</strong> <code><?php echo $password ? str_repeat('*', strlen($password)) : '(empty)'; ?></code></p>
            <p><strong>Database:</strong> <code><?php echo $database; ?></code></p>
            <p><em>Note: You can modify these settings at the top of this file if needed.</em></p>
        </div>
...
try {
    ...
    echo '<div class="step">🗃️ <strong>Step 2:</strong> Creating database ' . $database . '...</div>';
    $pdo->exec("CREATE DATABASE IF NOT EXISTS `$database` DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci");
    $pdo->exec("USE `$database`");
    echo '<div class="step success">✅ Database created/selected successfully!</div>';
    
    // Step 3: Set SQL mode and time zone
    echo '<div class="step">⚙️ <strong>Step 3:</strong> Setting SQL configuration...</div>';
    $pdo->exec('SET SQL_MODE = "NO_AUTO_VALUE_ON_ZERO"');
    $pdo->exec('SET time_zone = "+00:00"');
    $pdo->exec('SET NAMES utf8mb4');
    echo '<div class="step success">✅ SQL configuration set successfully!</div>';
    
    // Step 4: Create tables
    echo '<div class="step">🔨 <strong>Step 4:</strong> Creating database tables...</div>';
    ...
    $pdo->exec($adminTable);
    echo '<div class="step success">✅ Admin table created successfully!</div>';
    ...
    $pdo->exec($usersTable);
    echo '<div class="step success">✅ Users table created successfully!</div>';
    
    // Clinics table
    $clinicsTable = "CREATE TABLE `clinics` (
        ...
        KEY `idx_clinic_email` (`clinic_email`),
        KEY `idx_clinic_status` (`status`),
        KEY `idx_clinic_location` (`location`(100))
    ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci";
    
    $pdo->exec($clinicsTable);
    echo '<div class="step success">✅ Clinics table created successfully!</div>';
    
    // Doctor table
    $doctorTable = "CREATE TABLE `doctor` (
    ...
    $pdo->exec($doctorTable);
    echo '<div class="step success">✅ Doctor table created successfully!</div>';
    
    // Doctor clinic assignments table
    $doctorClinicTable = "CREATE TABLE `doctor_clinic_assignments` (
        `id` int(11) NOT NULL AUTO_INCREMENT,
        ...
        CONSTRAINT `doctor_clinic_assignments_ibfk_1` FOREIGN KEY (`doctor_id`) REFERENCES `doctor` (`doc_id`) ON DELETE CASCADE,
        CONSTRAINT `doctor_clinic_assignments_ibfk_2` FOREIGN KEY (`clinic_id`) REFERENCES `clinics` (`clinic_id`) ON DELETE CASCADE
    ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci";
    
    $pdo->exec($doctorClinicTable);
    echo '<div class="step success">✅ Doctor clinic assignments table created successfully!</div>';
    ...
    $appointmentsTable = "CREATE TABLE `appointments` (
        ...
        KEY `idx_doctor_date_time` (`doctor_id`,`appointment_date`,`appointment_time`),
        KEY `idx_patient_email` (`patient_email`),
        KEY `idx_appointment_datetime` (`appointment_date`,`appointment_time`),
        KEY `unique_user_appointment_slot` (`doctor_id`,`appointment_date`,`appointment_time`,`booked_by_email`) USING BTREE,
        CONSTRAINT `fk_appointments_clinic` FOREIGN KEY (`clinic_id`) REFERENCES `clinics` (`clinic_id`) ON DELETE SET NULL,
        CONSTRAINT `fk_appointments_doctor` FOREIGN KEY (`doctor_id`) REFERENCES `doctor` (`doc_id`) ON DELETE CASCADE
    ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci";
    
    $pdo->exec($appointmentsTable);
    echo '<div class="step success">✅ Appointments table created successfully!</div>';
    
    // Lab tests table
    $labTestsTable = "CREATE TABLE `lab_tests` (
        `id` int(11) NOT NULL AUTO_INCREMENT,
    ...
    $pdo->exec($labTestsTable);
    echo '<div class="step success">✅ Lab tests table created successfully!</div>';
    
    // Lab orders table
    $labOrdersTable = "CREATE TABLE `lab_orders` (
        `id` int(11) NOT NULL AUTO_INCREMENT,
        ...
        PRIMARY KEY (`id`),
        KEY `clinic_id` (`clinic_id`),
        CONSTRAINT `clinic_id` FOREIGN KEY (`clinic_id`) REFERENCES `clinics` (`clinic_id`)
    ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci";
    
    $pdo->exec($labOrdersTable);
    echo '<div class="step success">✅ Lab orders table created successfully!</div>';
    
    // Lab order items table
    $labOrderItemsTable = "CREATE TABLE `lab_order_items` (
        `id` int(11) NOT NULL AUTO_INCREMENT,
    ...
    $pdo->exec($labOrderItemsTable);
    echo '<div class="step success">✅ Lab order items table created successfully!</div>';
    
    // Medicines table
    $medicinesTable = "CREATE TABLE `medicines` (
        `id` int(11) NOT NULL AUTO_INCREMENT,
        `name` varchar(255) NOT NULL,
    ...
    $pdo->exec($medicinesTable);
    echo '<div class="step success">✅ Medicines table created successfully!</div>';
    
    // Medicine orders table
    $medicineOrdersTable = "CREATE TABLE `medicine_orders` (
        `id` int(11) NOT NULL AUTO_INCREMENT,
    ...
    $pdo->exec($medicineOrdersTable);
    echo '<div class="step success">✅ Medicine orders table created successfully!</div>';
    
    // Medicine order items table
    $medicineOrderItemsTable = "CREATE TABLE `medicine_order_items` (
        `id` int(11) NOT NULL AUTO_INCREMENT,
        `order_id` int(11) NOT NULL,
    ...
    $pdo->exec($medicineOrderItemsTable);
    echo '<div class="step success">✅ Medicine order items table created successfully!</div>';
    
    // Step 5: Insert sample data
    echo '<div class="step">📊 <strong>Step 5:</strong> Inserting sample data...</div>';
    
    // Insert admin data
    $adminData = "INSERT INTO `admin` (`adm_id`, `adm_name`, `adm_email`, `adm_ph`, `adm_img`, `adm_pass`) VALUES
        (1, 'Admin', 'admin@gmail.com', 0, '', 'admin123')";
    $pdo->exec($adminData);
        
    // Insert lab tests data
    $labTestsData = "INSERT INTO `lab_tests` (`id`, `name`, `sample_type`, `price`, `description`) VALUES
    ...
    $pdo->exec($labTestsData);
    
    // Insert medicines data
    $medicinesData = "INSERT INTO `medicines` (`id`, `name`, `manufacturer_name`, `pack_size`, `composition1`, `composition2`, `price`) VALUES
    ...
    $pdo->exec($medicinesData);
    
    echo '<div class="step success">✅ Sample data inserted successfully!</div>';
    
    // Step 6: Set AUTO_INCREMENT values
    echo '<div class="step">🔢 <strong>Step 6:</strong> Setting AUTO_INCREMENT values...</div>';
    ...
    $autoIncrements = [
        ...
        'clinics' => 8,
        'doctor' => 16,
        'doctor_clinic_assignments' => 4,
        'lab_orders' => 5,
        'lab_order_items' => 5,
        'lab_tests' => 398,
        'medicine_orders' => 31,
        'medicine_order_items' => 2,
        'users' => 15
    ];
    ...
    echo '<div class="step success">✅ AUTO_INCREMENT values set successfully!</div>';
    
    // Installation completed
    echo '<div class="step success">🎉 <strong>Installation Completed Successfully!</strong></div>';
    echo '<div class="step success">✅ Database "' . $database . '" has been created with all tables and sample data!</div>';
    
    // Display connection info
    echo '<div class="config-section">';
    echo '<h3>📋 Connection Information</h3>';
    echo '<p>You can now connect to your database using:</p>';
    echo '<ul>';
    echo '<li><strong>Database:</strong> ' . $database . '</li>';
    echo '<li><strong>Admin Login:</strong> admin@gmail.com / admin123</li>';
    echo '<li><strong>Total Tables Created:</strong> 11</li>';
    echo '<li><strong>Sample Data:</strong> Included</li>';
    echo '</ul>';
    echo '</div>';
    
    echo '<div class="step warning">⚠️ <strong>Important:</strong> Please delete this install.php file after successful installation for security reasons!</div>';
    
} catch (PDOException $e) {
    echo '<div class="step error">❌ <strong>Installation Failed!</strong></div>';
    echo '<div class="step error">Error: ' . $e->getMessage() . '</div>';
    echo '<div class="step warning">Please check your database configuration and try again.</div>';
} catch (Exception $e) {
    echo '<div class="step error">❌ <strong>Unexpected Error!</strong></div>';
    echo '<div class="step error">Error: ' . $e->getMessage() . '</div>';
}
?>

        <div style="text-align: center; margin-top: 30px;">
            <a href="<?php echo $_SERVER['PHP_SELF']; ?>" class="btn">🔄 Run Installation Again</a>
            <a href="http://localhost/cure_booking/adminhub/index.php" class="btn">🏠 Go to Application</a>
        </div>
        
        <div style="text-align: center; margin-top: 20px; color: #6c757d; font-size: 14px;">
            <p>© 2025 Cure Booking System - Database Installation Script</p>
            <?php echo 'Server: ' . date('Y-m-d H:i:s') . ' | Local: ' . date('Y-m-d H:i:s', time() + (5.5 * 3600));?>
        </div>
    </div>
</body>
</html>