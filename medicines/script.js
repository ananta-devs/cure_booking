// Optimized JavaScript code for medicine ordering system
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    function setupEventListeners() {
        ...
        // Cart buttons
        const cartButtons = ["view-cart-btn", "checkout-btn", "clear-cart-btn", "proceed-checkout-btn"];
        const cartActions = [
            () => openModal("cart"),
            () => openModal("order"), 
            clearCart,
            () => { closeModal("cart"); openModal("order"); }
        ];

        cartButtons.forEach((id, index) => {
            const btn = $(id);
            if (btn) {
                const action = index < 2 ? () => checkLoginStatus() && cartActions[index]() : cartActions[index];
    ...
    function updateCartDisplay() {
        if (!elements.cartCount || !elements.cartTotal || !elements.modalCartTotal) return;
    ...
    function updateCartButtonState(button, medicineId) {
        ...
        const config = isInCart 
            ? { html: '<i class="ri-check-line"></i> Added', class: 'add-to-cart-btn in-cart', bg: '#374151', disabled: true }
            : { html: '<i class="ri-shopping-cart-line"></i> Add to Cart', class: 'add-to-cart-btn', bg: '#3B82F6', disabled: false };
    ...
    function getCompositionText(medicine) {
        const compositions = [medicine.composition1, medicine.composition2].filter(Boolean);
        return compositions.length ? compositions.join(", ") : "Composition not available";
    }
