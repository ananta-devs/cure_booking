...
function initializeEventListeners() {
    ...
    document
        ...
        ?.addEventListener("click", cancelAppointmentForm);
    document
    ...
    document.addEventListener(
        "keydown",
        (e) => e.key === "Escape" && closeDoctorModal()
    );
...
function viewAppointment(appointmentId) {
    const row = Array.from(
        document.querySelectorAll("#appointmentsTableBody tr")
...
async function populateUpdateModal(appointmentData) {
    try {
        ...
        // Set gender
        document
            .querySelectorAll('input[name="updateGender"]')
            .forEach((radio) => {
        ...
        document.getElementById("updateSpecialityType").value =
        ...
        const doctorOption = Array.from(doctorSelect.options).find((option) =>
            option.textContent.includes(appointmentData.doctor)
        );
        ...
        if (doctorOption) {
            ...
            if (formattedDate) {
                await loadTimeSlots(
                    ...
                    formattedDate,
                    "update",
                    currentEditAppointmentId
                );
                await new Promise((resolve) => setTimeout(resolve, 200));

                const timeSelect = document.getElementById("updateTime");
                const timeOption = Array.from(timeSelect.options).find(
                    (option) =>
...
function handleDoctorChange(isUpdate = false) {
    ...
    if (doctorInfo) {
        const selectedOption = document.querySelector(
            `#${prefix}Doctor option[value="${doctorId}"]`
        );
        doctorInfo.innerHTML = selectedOption
            ? `<small>Selected: ${selectedOption.textContent}</small>`
            : "";
    }
...
function loadTimeSlots(
    ...
    if (excludeAppointmentId)
        formData.append("exclude_appointment_id", excludeAppointmentId);
    ...
    return fetch("api.php", { method: "POST", body: formData })
        ...
        .then((data) => {
            ...
            if (data.success && data.time_slots?.length) {
                ...
                if (!data.success)
                    showMessage(
                        data.message || "Error loading time slots",
                        "error"
                    );
            }
        })
...
function handleAppointmentSubmit(event) {
    ...
    formData.append(
        "action",
        isEditMode ? "update_appointment" : "book_appointment"
    );
...
function submitUpdateAppointment() {
    ...
    formData.append(
        "appointment_date",
        document.getElementById("updatePreferredDate").value
    );
    formData.append(
        "appointment_time",
    ...
    modalBody.innerHTML = `
        <div class="doctor-details">
            <div class="doctor-header">
                <img src="${
                    doctorData.doc_img
                        ? "uploads/doctors/" + doctorData.doc_img
            ...
            <div class="doctor-info-grid">
                <div class="info-item"><strong>Email:</strong><span>${
                    doctorData.doc_email
                }</span></div>
                <div class="info-item"><strong>Location:</strong><span>${
                    doctorData.location
                }</span></div>
                <div class="info-item"><strong>Education:</strong><span>${
                    doctorData.education
                }</span></div>
                <div class="info-item"><strong>Specialization:</strong><span>${
                    doctorData.doc_specia
                }</span></div>
            </div>
        </div>
    `;
...
function validateFormFields(fields, prefix = "") {
    ...
    if (
        phoneField?.value &&
        !/^[0-9]{10}$/.test(phoneField.value.replace(/\D/g, ""))
    ) {
        isValid = false;
...
function resetForm() {
    ...
    form.reset();
    form.querySelectorAll(".error").forEach(
...
function filterAppointments() {
    ...
    rows.forEach((row) => {
        ...
        if (filterDate) {
            ...
            showRow =
                showRow &&
                rowDate.toDateString() === filterDateObj.toDateString();
        }

        if (filterStatus) {
            showRow =
                showRow && statusCell.includes(filterStatus.toLowerCase());
        }
