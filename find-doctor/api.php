    ...
    foreach ($session_keys as $key => $possible_keys) {
        foreach ($possible_keys as $session_key) {
            if (isset($_SESSION[$session_key]) && !empty($_SESSION[$session_key])) {
                ...
                if ($key === 'user_id' && $_SESSION[$session_key] > 0) {
                    $logged_in = true;
                }
                if ($key === 'user_email') {
                    $logged_in = true;
...
function bookAppointment($conn) {
    ...
    try {
        ...
        if (!$stmt_doctor) {
            throw new Exception("Prepare failed for doctor query: " . $conn->error);
        }
        ...
        // Rate limiting check - limit 3 bookings per hour per user
        ...
        if (!$stmt_rate) {
            throw new Exception("Prepare failed for rate limit check: " . $conn->error);
        }
        ...
        if (!$stmt_existing) {
            throw new Exception("Prepare failed for existing appointment check: " . $conn->error);
        }
        ...
        error_log("Available columns in appointments table: " . implode(', ', $columns));
        ...
        error_log("Values: " . print_r(array_values($insert_columns), true));
...
function getDoctorAvailability($conn) {
    try {
        ...
        // If no clinic-specific availability found, try to get general doctor availability
