    ...
    clinicNames.forEach((clinicName, index) => {
        ...
        clinicInfoHTML += `
            <div class="clinic-item">
                ...
                ${clinicAvailability ? `<div class="clinic-availability">${clinicAvailability}</div>` : ''}
            </div>
        `;
    });
...
function formatAvailabilitySchedule(schedule) {
    ...
    dayOrder.forEach(day => {
        if (schedule[day] && Object.keys(schedule[day]).length > 0) {
            const slots = Object.keys(schedule[day]).filter(slot => schedule[day][slot]);
            if (slots.length > 0) {
                availableDays.push({
    ...
    formattedSchedule += '<h5><i class="fas fa-calendar-alt"></i> Weekly Schedule:</h5>';
    ...
    try {
        ...
        if (data.time_slots && data.time_slots.length > 0) {
            ...
            if (hasAvailableSlots) {
                ...
                timeSelect.innerHTML = '<option value="">All slots are full for this date</option>';
                timeSelect.disabled = true;
            }
        } else {
        ...
        showNotification('Failed to load time slots. Please try again.', 'error');
    }
}
