...
<body>
    ...
    <script>
        ...
        function renderClinicAssignments(assignments) {
            ...
            assignments.forEach(assignment => {
                ...
                weekDays.forEach(day => {
                    html += `<tr><td style="border: 1px solid #ddd; padding: 8px; font-weight: bold;">${day.charAt(0).toUpperCase() + day.slice(1)}</td>`;

                    timeSlots.forEach(slot => {
                        const isAvailable = assignment.availability_schedule && assignment.availability_schedule[day] && assignment.availability_schedule[day][slot];
                ...
                html += `</tbody></table></div></div><hr style="margin: 20px 0;">`;
            });
            ...
            fetch("get_doctor.php?action=get_clinics")
                ...
                .then(data => {
                    if (data.status === "success") {
                        ...
                        availableClinics = [];
                        showMessage("warning", "Could not load clinics list. " + (data.message || ""));
                    }
                })
                .catch(error => {
                    console.error("Error loading clinics:", error);
                    availableClinics = [];
                    showMessage("warning", "Failed to load clinics list. Please refresh the page.");
                });
        }
        ...
        function loadClinicAssignments(assignments) {
            ...
            document.getElementById("add_clinic_assignment").onclick = () => addClinicAssignmentRow(null, assignments.length);
        }
        ...
        function createClinicOptions(assignment = null, excludeAllocated = false) {
            ...
            if (availableClinics.length > 0) {
                ...
                availableClinics.forEach(clinic => {
                    const isCurrentClinic = assignment && assignment.clinic_id == clinic.clinic_id;

                    if (excludeAllocated && allocatedIds.includes(clinic.clinic_id.toString()) && !isCurrentClinic) {
                        return;
                    }
        ...
        function addClinicAssignmentRow(assignment = null, index = 0) {
            ...
            weekDays.forEach(day => {
                scheduleHtml += `<tr><td style="border: 1px solid #ddd; padding: 8px; font-weight: bold;">${day.charAt(0).toUpperCase() + day.slice(1)}</td>`;

                timeSlots.forEach(slot => {
                    const isChecked = assignment && assignment.availability_schedule && assignment.availability_schedule[day] && assignment.availability_schedule[day][slot] ? "checked" : "";
                    scheduleHtml += `<td style="border: 1px solid #ddd; padding: 8px; text-align: center;">
                        <input type="checkbox" name="clinic_assignments[${index}][availability][${day}][${slot}]" value="1" ${isChecked}>
                    </td>`;
                });
                scheduleHtml += "</tr>";
            });
            ...
            const newSelect = container.querySelector(`.clinic-assignment-row:last-child .clinic-select`);
            if (newSelect) {
                newSelect.addEventListener("change", () => updateAllClinicDropdowns());
            }
        }
        ...
        function updateAllClinicDropdowns() {
            ...
            assignmentRows.forEach(row => {
                ...
                availableClinics.forEach(clinic => {
                    ...
                    const isAlreadyAllocated = allocatedIds.includes(clinicId) && !isCurrentSelection;
